...
const createProduct = async (request, response) => {
    try{
        const {productName,actualPrice, oldPrice, qty, description, file, discount, categoryId} = request.body;
        if (!productName || !actualPrice || !oldPrice || !qty || !description || !file || !discount || !categoryId){
...
const findProductById = async (request, response) => {
    try{
        ...
        if (productData){
            return response.status(200).json({code:200, message:'product data...', data:countryData});
        }
